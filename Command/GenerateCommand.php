...
class GenerateCommand extends ContainerAwareCommand
{
     ...
     * @var array|string
     */
    protected $corePlugins = array(
        'validation' => 'Ano\\GeneratorBundle\\Command\\Plugin\\ValidationConstraintsPlugin',
    );
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption('default-type', 't', InputOption::VALUE_OPTIONAL, 'Class members default type', 'string')
            ->addOption('default-access-level', 'l', InputOption::VALUE_OPTIONAL, 'Class members default access level', 'private')
            ->addOption('default-getter', 'g', InputOption::VALUE_OPTIONAL, 'Whether or not class member getter should be generated by default', 0)
            ->addOption('default-setter', 'm', InputOption::VALUE_OPTIONAL, 'Whether or not class member setter should be generated by default', 0)
            ->addOption('plugins', 'p', InputOption::VALUE_OPTIONAL, 'Core plugins to use (separated by commas)', null)
            ->setHelp('');
    }
...
//            var_export($this->members, true),
    ...
    protected function loadPlugins(InputInterface $input)
    {
        //TODO: load user defined plugins
        $this->plugins[] = new GenericObjectPlugin();

        $plugins = array();
        if ($input->getOption('plugins')) {
            $plugins = explode(',', $input->getOption('plugins'));
        }

        foreach($plugins as $plugin) {
            if (!isset($this->corePlugins[$plugin])) {
                throw new \InvalidArgumentException(sprintf('Plugin "%s" doesn\'t exist.', $plugin));
            }

            $pluginClass = $this->corePlugins[$plugin];
    ...
    protected function loadOptions(InputInterface $input)
    {
        //TODO: $this->options = $this->getContainer()->getParameter('ano_generator.config');
        $this->options = array(
            'member' => array(
                'defaults' => array(
                    'type' => $input->hasOption('default-type') ? $input->getOption('default-type') : 'string',
                    'access_level' => $input->hasOption('default-access-level') ? $input->getOption('default-access-level') : 'private',
                    'getter' => $input->hasOption('default-getter') ? (bool) $input->getOption('default-getter') : true,
                    'setter' => $input->hasOption('default-setter') ? (bool) $input->getOption('default-setter') : true,
                )
            )
        );
    }
